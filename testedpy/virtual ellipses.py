...
#win to show stimuli
win = visual.Window((1024, 768), units='pix', fullscr = False, allowStencil=True)
...
当判断重叠时，和任意一个点重叠都应该continue这个while循环(此处为函数的return)，得到新的随机点
...
'''
For each disks, caclulate the tangential ellipse as well.
Then caclulate the overlapping area>>>>four intersections
'''